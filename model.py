...
class GCN(nn.Module):
    ...
    def renyi_loss(self, epsilon=0.01, lambda_reg=0.1, adj=None):
        entropy_reg = 0.0
        for hidden in self.hidden_states:
            hidden_norm = F.normalize(hidden, p=2, dim=1)
            sim_matrix = torch.mm(hidden_norm, hidden_norm.t())
            kernel_matrix = torch.exp(-torch.pow(1 - sim_matrix, 2) / (2 * epsilon ** 2))*adj
            n_samples = hidden.size(0)
            V = torch.sum(kernel_matrix) / (n_samples ** 2)
            entropy = -torch.log(V)
            entropy_reg -= entropy
        entropy_reg = entropy_reg / len(self.hidden_states)
        return entropy_reg
...
class GCNII(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        self.hidden_states.append(layer_inner)
        _layers = [layer_inner]
...
class GPRGNN(torch.nn.Module):
    ...
    def renyi_loss(self, epsilon=0.01, lambda_reg=0.1, adj=None):
        ...
        for hidden in self.hidden_states:
            hidden_norm = F.normalize(hidden, p=2, dim=1)
            sim_matrix = torch.mm(hidden_norm, hidden_norm.t())
            # si tiene 2xE lo pasamos a NxN la adj
            if adj.size(0) == 2:
                adj = to_dense_adj(adj).squeeze(0).to(hidden.device)
            kernel_matrix = torch.exp(-torch.pow(1 - sim_matrix, 2) / (2 * epsilon ** 2))*adj
            n_samples = hidden.size(0)
            V = torch.sum(kernel_matrix) / (n_samples ** 2)
            entropy = -torch.log(V)
...
class GGCN(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        self.hidden_states.append(layer_previous)
