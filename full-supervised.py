...
parser.add_argument('--renyi', type=float, default=0.0, help='Renyi entropy parameter')
parser.add_argument('--csv_output', type=str, default='gnn_results.csv', help='CSV file to save results')
...
def train_step(model,optimizer, features, labels, adj, idx_train, use_geom):
    ...
    loss_train = F.nll_loss(output[idx_train], labels[idx_train].to(device)) + args.renyi * model.renyi_loss(adj=adj)
    loss_train.backward()
    optimizer.step()
...
def save_results_to_csv(model, layers, renyi, test_acc, test_std, output_file):
    """
    Save the GNN model results to a CSV file
    """
    ...
    # Comprobar si el archivo ya existe para añadir o crear nuevo
    ...
    # También imprimir los resultados para verificación
    print("\nSaved results:")
    print(df)
...
save_results_to_csv(
    ...
    layers=args.layer,
    renyi=args.renyi,
    test_acc=round(np.mean(acc_list),2),
    test_std=round(np.std(acc_list),2),
    output_file="results/"+args.model+"_"+args.data+"_results.csv"
)